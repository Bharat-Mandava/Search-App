...
const getCountriuesWithBlank = async (e) => {
    ...
    const res = await axios.get(`/fullScreen`);
...
const outputHtml = (matches) => {
    ...
    if (matches.length > 0) {
        const html = matches.map((match) => `
                ...
                <div style="width:100%" class="spanContent">${match.country_code}<br><span style="font-size:13px; display: ${match.imgURL === "0" ? "none" : "block"}">This is a country</span>
                </div>
